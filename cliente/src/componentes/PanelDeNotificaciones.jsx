...
// en const data tienes que hacer el metodo para que lleguen las notificaciones, por ejemplo:
const data = [
  {
    ...
    text: "Una nueva tarea ha sido asignada. la fecha limite es el 30 de Abril 2024",
    task: null,
    notiType: "alerta",
    isRead: [],
...
const ICONS = {
  alerta: (
    <HiBellAlert className='h-5 w-5 text-gray-600 group-hover:text-indigo-600 notificaciones-Campana-Color bg-gray-100' />
  ),
  message: (
      ...
      <Popover className='relative'>
        <PopoverButton className='inline-flex items-center outline-none'>
          <div className='w-8 h-8 flex items-center justify-center text-gray-800 relative'>
            ...
            {/*aqui si hay notificaciones activa el circulo que indica que hay notificaciones*/}
            {data?.length > 0 && (
                ...
                <div className='w-screen max-w-md flex-auto overflow-hidden rounded-3xl bg-white text-sm leading-6 shadow-lg ring-1 ring-gray-900/5'>
                  <div className='p-4'>
                    {data?.slice(0, 5).map((item, index) => (
                      ...
                      >
                        <div className='mt-1 h-8 w-8 flex items-center justify-center rounded-lg bg-gray-200 group-hover:bg-white color'>
                          {ICONS[item.notiType]}
                        </div>
...
export default PanelDeNotificaciones;